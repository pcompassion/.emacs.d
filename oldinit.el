...
;; ‚ÄÏÎ§èlways‚Äù means no asking.
;; ‚ÄÏÎ¶∂op‚Äù means ask once (top = top dir).
;; any other symbol means ask each and every time for a dir and subdir.
(setq dired-recursive-copies (quote always))
(setq dired-recursive-deletes (quote top))

;; The bigger problem is that the header-search algorithm doesn‚ÄÏÎÄΩ understand frameworks. So, <sys/types> properly finds /usr/include/sys/types, but <Foundation/Foundation.h> fails. To fix that:
...
;; Of course you‚ÄÏÎÄ¥l also want to add the framework paths to cc-search-directories. I used to have code to extract the gcc defaults for both include directories and framework directories at startup, but that turns out to be not much better than using the built-in defaults, because it doesn‚ÄÏÎÄΩ take into account the local values for the project(s) I work on. So now I just hard-code it in my .emacs, and then have project-specific files to eval as needed.

(setq cc-search-directories '("." "../include" "/usr/include" "/usr/local/include/*"
			            "/System/Library/Frameworks" "/Library/Frameworks"))


;; So, any .h file that defines an @interface is objc-mode instead of c-mode. (You could extend this to distinguish C++ from C by looking for class or template definitions, but I haven‚ÄÏÎÄΩ bothered.)
